...
class _AttendancePageState extends ConsumerState<AttendancePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final useWebVpn =
        system.usesLegacyApi && ref.read(campusSessionProvider).useWebVpn;
    final data = value.asData?.value;
    final error = _refreshFailure ?? (value.hasError ? value.error : null);
    final records = (data?.records ?? const <AttendanceRecord>[]).where((
      record,
    ) {
      final matchDate =
          _range == null ||
          (!record.date.isBefore(_range!.start) &&
              !record.date.isAfter(_range!.end));
      return matchDate &&
          (_status == null || record.status == _status) &&
          '${record.courseName} ${record.location} ${record.teacher}'
              .toLowerCase()
              .contains(_keyword.toLowerCase());
    }).toList();
    return AppPageScaffold(
      appBar: AppBar(
        title: const Text('考勤查询'),
        actions: [
          IconButton(
            tooltip: '接口诊断',
            onPressed: () => showAttendanceDiagnostics(context),
          ...
          itemBuilder: (context, index) {
            if (index == 0)
              return Column(
                ...
                children: [
                  ServiceStatusBanner(
                    service: 'attendance',
                    onRetry: _refreshAttendance,
                    loginRoute: '/attendance/login',
                  ),
                  const SizedBox(height: 12),
                  SegmentedButton<AttendanceSystem>(
                    segments: [
                      for (final item in AttendanceSystem.values)
                    ...
                    onSelectionChanged: (selection) {
                      ...
                      ref
                          .read(attendanceSystemProvider.notifier)
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          data == null
                              ? '本学期考勤'
                              : '${data.term} · ${data.records.length} 条记录',
                          style: Theme.of(context).textTheme.titleMedium,
                        ),
                        const SizedBox(height: 8),
                        Text(
                          data == null
                              ? '同步学校考勤记录，不将未返回的记录推断为缺勤。'
                    ...
                    children: [
                      ...
                      ActionChip(
                        label: Text(
                          _range == null
                              ? '筛选日期'
                              : '${DateFormat('M/d').format(_range!.start)}–${DateFormat('M/d').format(_range!.end)}',
                        ),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (error is AttendanceAccountRequired)
                            FilledButton(
                              onPressed: _openSchoolLogin,
                              child: const Text('登录学校账号'),
                            )
                          else if (error is AttendanceAuthRequired)
                            FilledButton(
                              onPressed: _openOfficial,
                              child: const Text('打开官方考勤系统'),
                            )
                          else
                            FilledButton(
                              onPressed: error is AttendanceAuthenticating
                                  ? null
                                  : _refreshAttendance,
                          ...
                          if (system.usesLegacyApi && !useWebVpn)
                            TextButton(
                              onPressed: () => context.push('/webvpn'),
                              child: const Text('连接 WebVPN'),
                            ),
                          if (error is! AttendanceAccountRequired &&
                  ...
                  if (data != null && records.isEmpty)
                    const Padding(
                      ...
                      child: Text('当前筛选范围暂无记录', textAlign: TextAlign.center),
                    ),
                ],
              );
            ...
            return AppSurfaceCard(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    record.courseName.isEmpty ? '课程考勤' : record.courseName,
                    style: Theme.of(context).textTheme.titleMedium,
                  ),
                  const SizedBox(height: 8),
                  Text(
                    '${DateFormat('M月d日').format(record.date)} · '
                    '${record.startPeriod > 0 && record.endPeriod >= record.startPeriod ? '第${record.startPeriod}–${record.endPeriod}节' : '节次未知'}',
                  ),
                  Text(
                    [
                      record.location,
