...
class BackgroundState extends Notifier<bool> {
  ...
  Future<bool> toggle(bool value) async {
    if (!(!kIsWeb && defaultTargetPlatform == TargetPlatform.android))
      return false;
    final result =
        await campusPlatform.invokeMethod<bool>('background', value) ?? false;
    if (ref.mounted) state = result;
    return result;
  }
}

final backgroundRunningProvider = NotifierProvider<BackgroundState, bool>(
  BackgroundState.new,
);
final widgetBookingsProvider = FutureProvider<List<LibraryBooking>>((
  ref,
) async {
  ...
  return session.readQueue.run(
    () => ref.read(librarySeatsRepositoryProvider).myBookings(),
  );
...
class _CampusPlatformSyncState extends ConsumerState<CampusPlatformSync>
  ...
  void initState() {
    ...
    campusPlatform.setMethodCallHandler((call) async {
      ...
      if (call.method == 'serviceStopped')
        ref.read(backgroundRunningProvider.notifier).update(false);
      if (call.method == 'refresh') await _refresh();
    });
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      try {
        _route(await campusPlatform.invokeMethod<String>('initialRoute'));
      } catch (_) {}
      await _status();
    });
  }
  ...
  Future<void> _status() async {
    try {
      final running =
          await campusPlatform.invokeMethod<bool>('status') ?? false;
  ...
  Future<void> _refresh() async {
    ...
    if (_refreshing ||
        user.isGuest ||
        user.isDemo ||
        ref.read(campusConnectionBusyProvider) ||
        ref.read(campusPreloadProvider).busy)
      return;
    _refreshing = true;
    try {
      ...
      await Future.wait([
        ref.read(scheduleSnapshotProvider.future),
        ref.read(attendanceSnapshotProvider.future),
        ref.read(homeworkProvider.future),
        ref.read(campusCardSnapshotProvider.future),
        ref.read(widgetBookingsProvider.future),
      ]).timeout(const Duration(minutes: 3));
    } catch (_) {
      /* Each service preserves its own cached data and failure UI. */
    } finally {
      _refreshing = false;
    }
  }
  ...
  void dispose() {
    ...
    if ((!kIsWeb && defaultTargetPlatform == TargetPlatform.android))
      campusPlatform.setMethodCallHandler(null);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (settings.widgets &&
      ...
      if (schedule != null && (schedule.live || schedule.fromCache)) {
        for (var i = 0; i < 7; i++) {
          ...
          final courses = coursesOnDay(
            schedule.courses,
            day,
            fallbackWeek: schedule.week,
          );
          final due =
              homework?.items.where((item) {
              ...
              0;
          final row =
              '${['周一', '周二', '周三', '周四', '周五', '周六', '周日'][i]} ${day.month}/${day.day}  ${courses.isEmpty ? '无课' : courses.map((c) {
          ...
          data['today$i'] =
              ...
              ? '1'
              : '0';
          data['lessons$i'] = courses.isEmpty
              ? '无课程安排'
              : courses
                    .map((c) {
                      final record = attendance?.forCourse(c, day);
        ...
        data['week'] =
            '${monday.month}/${monday.day} 起 · ${schedule.fromCache ? '缓存课表' : '本周课程'}';
      }
      if (attendance != null)
        data['attendance'] =
            '考勤 · 本学期 ${attendance.records.length} 条${attendance.fromCache ? '（缓存）' : ''}';
      if (homework != null) {
        final upcoming = homework.items
            .where(
              (a) =>
                  a.status != HomeworkStatus.submitted &&
                  a.dueAt != null &&
                  a.dueAt!.isAfter(now),
            )
            .toList();
        data['homework'] =
            '作业 · ${upcoming.isEmpty ? '暂无已知待截止作业' : '${DateFormat('M/d HH:mm').format(campusTime(upcoming.first.dueAt!))} ${upcoming.first.title}'}${homework.fromCache ? '（缓存）' : ''}';
      }
      if (card?.card != null && (card!.live || card.fromCache))
        data['card'] =
            '校园卡 · ¥${card.card!.balanceLabel}${card.fromCache ? '（缓存）' : ''}';
      final seats = bookings.asData?.value;
      if (seats != null)
        data['seat'] = seats.isEmpty
            ? '座位 · 暂无已知预约'
            : '座位 · ${seats.first.areaCode} ${seats.first.seatId} ${seats.first.statusLabel ?? ''}';
      final timestamps = [
        schedule?.cachedAt ?? schedule?.fetchedAt,
        attendance?.updatedAt,
        homework?.updatedAt,
        card?.cachedAt ?? card?.fetchedAt,
      ].whereType<DateTime>().toList()..sort();
      data['updated'] = timestamps.isEmpty
          ? '数据尚未同步 · 点击打开设置'
    ...
    if (_last != encoded) {
      ...
      _publishTimer = Timer(const Duration(milliseconds: 300), () async {
        try {
          await campusPlatform.invokeMethod('widget', data);
        } catch (_) {
          _last = null;
        }
      });
    ...
    if (auth.user.isGuest && ref.watch(backgroundRunningProvider)) {
      Future.microtask(() async {
        if (mounted) {
          try {
            await ref.read(backgroundRunningProvider.notifier).toggle(false);
          } catch (_) {}
        }
      });
    }
